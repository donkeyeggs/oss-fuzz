...
import torch
import tensorflow
cout = LOG.Log('max_pool1d_fuzzer',log_dir=TEST._INIT_DIR)
@st.composite
def input_data(draw):
    max_batch = 5
    max_channel = 5
    ...
    input_shape = (batch,in_channels,H_len)
    input = draw(TEST.ARRAY_ND(shape = input_shape))

    kernel_size= kH_len

    padding = draw(
        st.integers(min_value=0,max_value=kH_len//2)
    )
    stride = draw(
        st.one_of(
...
def test_paddle(_input,_kernel_size,_stride,_padding):
    ...
    stride = _stride
    padding = _padding
    output = paddle.nn.functional.max_pool1d(
        x=input,
        kernel_size=kernel_size,
        stride=stride,
        padding=padding,
...
def test_tensorflow(_input,_kernel_size,_stride,_padding):
    ...
    stride = _stride
    padding = _padding
    output = tensorflow.nn.max_pool1d(
        input = input, 
        ksize = kernel_size, 
        strides = stride, 
...
if __name__ == "__main__" and TEST.PLATFORM() == "linux":
    ...
    fuzz_target = atheris.instrument_func(test_max_pool1d.hypothesis.fuzz_one_input)
